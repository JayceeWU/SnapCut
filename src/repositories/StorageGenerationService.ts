...
/**
 * v7 intentionally starts a new private editing store. The marker is the sole
...
export class StorageGenerationService {
  ...
  async ensureCurrentGeneration(): Promise<boolean> {
    ...
    for (const uri of [
      ...
      if (this.layout.fileSystem.fileExists(uri)) {
        throw new Error('SnapCut v7 storage cleanup did not remove an app-owned metadata file.');
      }
    }
    this.deleteLegacyCorruptIndexFiles();
  ...
  private deleteDirectoryAndVerify(uri: string): void {
    ...
    if (this.layout.fileSystem.directoryExists(uri)) {
      throw new Error('SnapCut v7 storage cleanup left an app-owned directory behind.');
    }
  }

  private deleteLegacyCorruptIndexFiles(): void {
    for (const entry of this.layout.fileSystem.listDirectory(this.layout.rootUri)) {
      ...
      if (this.layout.fileSystem.fileExists(entry.uri)) {
        throw new Error('SnapCut v7 storage cleanup left a legacy index backup behind.');
      }
    }
  }
}
